<template>
  <div class="lattice-type">
    <canvas :id="canvasID" :style="{'width': `${width/2}px`, 'height': `${height/2}px`}" />
  </div>
</template>

<script>
import { rectInit } from '@/components/canvas/utils/logic';
  ...
  methods: {
    init() {
      ...
      for (let i = 0; i <= 9; i++) {
        ...
        this.height = y + this.w + this.m_b;
        this.point = [this.m_r, this.height + this.m_b * 2];
